...
final class DefaultCommunityUseCaseTests: XCTestCase {
    ...
    override func setUp() {
        ...
        useCase = DefaultCommunityUseCase(service: mockService)
    }
    ...
    func testFetchFollowingList() async throws {
        let userID = "user1"
        ...
        let results = try await useCase.fetchFollowingList(userID: userID)
    ...
    func testFetchMyFollowerList() async throws {
        let userID = "user1"
        mockService.fetchedRelations = [
            FollowRelationDTO(id: "relation1", date: Date(), following: "user2", follower: userID, state: false),
            FollowRelationDTO(id: "relation2", date: Date(), following: "user3", follower: userID, state: false)
        ]
        
        let results = try await useCase.fetchMyFollowerList(userID: userID)
